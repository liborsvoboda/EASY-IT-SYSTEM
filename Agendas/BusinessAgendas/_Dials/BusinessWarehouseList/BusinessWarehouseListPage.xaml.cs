...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class BusinessWarehouseListPage : UserControl {
        public static DataViewSupport dataViewSupport = new DataViewSupport();
        public static BusinessWarehouseList selectedRecord = new BusinessWarehouseList();

        public BusinessWarehouseListPage() {
            InitializeComponent();
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                DgListView.ItemsSource = await CommunicationManager.GetApiRequest<List<BusinessWarehouseList>>(ApiUrls.EasyITCenterBusinessWarehouseList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
            } catch (Exception autoEx) { App.ApplicationLogging(autoEx); }
        ...
        public void Filter(string filter) {
            try {
                ...
                DgListView.Items.Filter = (e) => {
                    BusinessWarehouseList user = e as BusinessWarehouseList;
        ...
        public void NewRecord() {
            selectedRecord = new BusinessWarehouseList();
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(true);
        }

        public void EditRecord(bool copy) {
            selectedRecord = (BusinessWarehouseList)DgListView.SelectedItem;
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(true, copy);
        }
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            ...
            else { selectedRecord = new BusinessWarehouseList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(false);
        }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, httpContent, null, App.UserData.Authentification.Token); }
        ...
        private void BtnCancel_Click(object sender, RoutedEventArgs e) {
            selectedRecord = (DgListView.SelectedItems.Count > 0) ? (BusinessWarehouseList)DgListView.SelectedItem : new BusinessWarehouseList();
            SetRecord(false);
        }
