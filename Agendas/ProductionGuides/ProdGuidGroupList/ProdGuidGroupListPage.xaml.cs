...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class ProdGuidGroupListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try { if (MainWindow.serviceRunning) DgListView.ItemsSource = await CommunicationManager.GetApiRequest<List<ProdGuidGroupList>>(ApiUrls.EasyITCenterProdGuidGroupList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token); } catch { }
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.EasyITCenterProdGuidGroupList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.EasyITCenterProdGuidGroupList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.EasyITCenterProdGuidGroupList, httpContent, null, App.UserData.Authentification.Token); }
