...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class BusinessPaymentStatusListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try { if (MainWindow.serviceRunning) DgListView.ItemsSource = await CommunicationManager.GetApiRequest<List<BusinessPaymentStatusList>>(ApiUrls.EasyITCenterBusinessPaymentStatusList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token); } catch (Exception autoEx) { App.ApplicationLogging(autoEx); }
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.EasyITCenterBusinessPaymentStatusList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.EasyITCenterBusinessPaymentStatusList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.EasyITCenterBusinessPaymentStatusList, httpContent, null, App.UserData.Authentification.Token); }
