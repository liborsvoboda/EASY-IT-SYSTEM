...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class SolutionUserRoleListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                userRoleLists = await CommunicationManager.GetApiRequest<List<SolutionUserRoleList>>(ApiUrls.EasyITCenterSolutionUserRoleList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.EasyITCenterSolutionUserRoleList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.EasyITCenterSolutionUserRoleList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.EasyITCenterSolutionUserRoleList, httpContent, null, App.UserData.Authentification.Token); }
